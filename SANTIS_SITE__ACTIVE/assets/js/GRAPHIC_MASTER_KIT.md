# SANTIS â€“ GRAPHIC MASTER KIT

**(Single Figma / PSD Structure)**

Bu belge, SANTIS markasının tüm görsel üretimleri için **kanonik (ana) yapı**yı tanımlar.

## 0) COVER & INDEX

*   **Kapak:** SANTIS logo + mood visual
*   **Index sayfasÄ± (tÄ±klanabilir):**
    *   Hero
    *   Cards
    *   Packages
    *   Face â€“ Sothys
    *   Kids & Family
    *   Social
    *   A4 / Print
    *   Icons
    *   Brand Tokens

---

## 1) BRAND TOKENS (Kilit Sayfa)

**Renkler (Global)**

*   `NV_BG` `#34393D` (Ana Zemin)
*   `NV_PANEL` `#41464A` (Ä°kincil Zemin / Kart)
*   `NV_FRAME` `#2C2F32` (Koyu Ã‡erçeve / Kenarlık)
*   `NV_DIVIDER` `#32363A` (AyÄ±rÄ±cÄ±)
*   `NV_STROKE` `#55595D` (Ä°nce Ã‡izgi)
*   `NV_HIGHLIGHT` `#9F9F9F` (Vurgu)

**Typography**

*   **Primary:** modern serif / soft grotesk
*   **Sizes:** H1â€“H6 / Body / Caption
*   **Letter-spacing rules:** luxury spacing (geniÅŸ aralÄ±k)

**Spacing & Radius**

*   **Spacing:** 4 / 8 / 16 / 24 / 32
*   **Radius:** 12 / 20 / 28

---

## 2) HERO & BANNER TEMPLATES

*   Home Hero (video / image)
*   Category Hero
*   Campaign Hero

**Variants:** Dark / Light, Center / Left aligned
**Notes:** Soft gradient overlay, Minimal text (1â€“2 satÄ±r)

---

## 3) CARD SYSTEM (CORE)

**Card Types:** Ritual Card, Massage Card, Package Card, Product Card, Kids Card
**States:** Default, Hover (very subtle), Selected, Disabled
**Elements:** Image, Title, Meta (duration / level), CTA

---

## 4) PACKAGE & COMBO LAYOUTS

*   2â€™li kombin
*   3â€™lÃ¼ ritÃ¼el akÄ±ÅŸÄ±
*   Signature / Prestige blok

**A4 & Web shared grid:** AynÄ± oran, aynÄ± ikon dili.

---

## 5) INFORMATION BLOCKS

*   RitÃ¼el AkÄ±ÅŸÄ± (1â€“2â€“3)
*   Kimler için ideal?
*   Faydalar
*   UyarÄ±lar

**Style:** Ä°nce Ã§izgi, minimal ikon, bol boÅŸluk.

---

## 6) FACE â€“ SOTHYS MODULES

*   Category cards
*   Treatment blocks
*   Comparison layout

**Tone:** Daha aydÄ±nlÄ±k, daha temiz, daha medikal-spa.

---

## 7) KIDS & FAMILY SAFE SET

*   Kids card
*   Family combo
*   Icon-safe illustrations

**Rules:** No adult poses, soft colors, friendly shapes.

---

## 8) SOCIAL MEDIA TEMPLATES

*   Instagram post (1:1)
*   Story (9:16)
*   Campaign announcement

**Overlay rules:** Text max %20, Logo minimum size.

---

## 9) A4 / PRINT TEMPLATES

*   Menü A4
*   Paket katalog
*   EÄŸitim kartlarÄ±

**Export ready:** 300 DPI, CMYK safe margins.

---

## 10) ICON & MICRO GRAPHICS

*   Massage type icons
*   Time / level icons
*   UI micro icons

---

## 11) EXPORT & USAGE RULES

*   Web export (SVG / WebP)
*   Print export (PDF)
*   Naming convention
*   Versioning (v1.0 / v1.1)

---

## Figma vs PSD Ã–neri

**Figma (önerilen):** Web + UI + component, Team work, Version control
**PSD:** Print aÄŸÄ±rlÄ±klÄ± iÅŸler, Foto manipulation

---

> **SonuÃ§:** SANTIS Grafik Master Kit, tÃ¼m web, sosyal medya, A4 ve kampanya grafiklerinin **tek dosyadan**, **tek dilde**, **tek standartla** Ã¼retilmesini saÄŸlar.