...
/* =========================
   SANTIS Prompt UI Ã¢â‚¬â€ app.js (RESET FINAL)
   ========================= */
...
function nvOpenReservationFromCard(item) {
    ...
    const program = item?.title || item?.name || "SANTIS Program";
    ...
    const source = "Source: SANTIS Prompt Library";
...
function wireEvents() {
    ...
    document.addEventListener("click", (e) => {
        ...
        if (mode === "json") {
            dl(`santis-prompt-${id}.json`, "application/json", JSON.stringify(item, null, 2));
        } else {
            dl(`santis-prompt-${id}.md`, "text/markdown", md(item));
        }
    });
    ...
    visibleMdBtn?.addEventListener("click", () => {
        ...
        dl("santis-visible-prompts.md", "text/markdown", out);
    });
    ...
    visibleJsonBtn?.addEventListener("click", () => {
        ...
        dl("santis-visible-prompts.json", "application/json", JSON.stringify(items, null, 2));
    });
}
